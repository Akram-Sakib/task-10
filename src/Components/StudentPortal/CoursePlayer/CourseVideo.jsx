...
import { updateSelectedVideo } from "../../../features/auth/authSlice";

const CourseVideo = ({ video }) => {
  const { id, title, views, duration } = video || {};
  const dispatch = useDispatch();

  const handleChangeSelectedVideo = () => {
    ...
    if (getAuth) {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...getAuth,
          user: { ...getAuth.user, selectedVideo: id },
        })
      );
      dispatch(updateSelectedVideo(id));
    }
  };

  return (
    <div
      onClick={handleChangeSelectedVideo}
